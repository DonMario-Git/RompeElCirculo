...
public class ButtonExtrasController : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler
{
    ...
    public float time_Up = 0.5f;
    public Ease ease_Up = Ease.OutElastic;
    ...
    {
        AudioPlayer.singleton.PlayVoiceClip(clip);
    }
    ...
    private void Update()
    {
        ...
        if (AppManager.data != null)
        {
            if (disableLicence)
            {
                SetInteractable(AppManager.data.activo);
            }
        }
